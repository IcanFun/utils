...
// Start and stop timer with no entries.
func TestNoEntries(t *testing.T) {
	...
	select {
	case <-time.After(OneSecond):
		t.Fatal("expected timer will be stopped immediately")
	case <-stop(cron):
	}
}
...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	case <-time.After(OneSecond):
		// No job ran!
	case <-wait(wg):
		t.Fatal("expected stopped timer does not run any job")
	}
}

// Add a job, start timer, expect it runs.
func TestAddBeforeRunning(t *testing.T) {
	...
	// Give timer 2 seconds to run our job (which is always activated).
	select {
	case <-time.After(OneSecond):
...
// Start timer, add a job, expect it runs.
...
// Add a job, remove a job, start timer, expect nothing runs.
...
// Start timer, add a job, remove it, expect it doesn't run.
...
func TestSnapshotEntries(t *testing.T) {
	...
	// Even though Entries was called, the timer should fire at the 2 second mark.
	select {
	case <-time.After(OneSecond):
...
// Test that the timer is run in the local time zone (as opposed to UTC).
...
// Test that the timer is run in the given time zone (as opposed to local).
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("repeated calls to stop, waiting for completion and after", func(t *testing.T) {
		...
		case <-ctx3.Done():
			// expected
		case <-time.After(time.Millisecond):
			t.Error("context not done even when timer Stop is completed")
		}
